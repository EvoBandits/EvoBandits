...
#     Please refer to the example shown
#     in :func:`~optuna.logging.disable_default_handler()`.
#     """
...
#     Note that log propagation is disabled by default. You only need to use this
#   function
...
#             logger.info("Logs from first optimize call")  # The logs are saved in
#           the logs file.
#             study.optimize(objective, n_trials=10)

#             optuna.logging.disable_propagation()  # Stop propogating logs to the
#           root logger.
...
#     Please disable the Optuna's default handler to prevent double logging if the root
#       logger has
#     been configured.
...
#             optuna.logging.enable_propagation()  # Propagate logs to the root logger.
#             optuna.logging.disable_default_handler()  # Stop showing logs in
#           sys.stderr.
